...
import { setInterviewStatusWithDate } from "@/lib/interview-actions";
...
export default function StatusSelector({ id, currentStatus }: StatusSelectorProps) {
  ...
  async function handleValueChange(value: string) {
    ...
    // If changing to Interview, show the date modal
    if (value === "Interview" && status !== "Interview") {
  ...
  async function handleInterviewDateSave(date: Date | null) {
    setShowDateModal(false);
    setIsLoading(true);
    setStatus("Interview"); // Optimistic update

    const result = await setInterviewStatusWithDate(id, date);
    ...
    if (result.error) {
      ...
      toast.success(date
        ? "Status updated to Interview with scheduled date"
  ...
  const getColor = (s: string) => {
    switch (s) {
      ...
      case 'Shortlisted': return 'bg-purple-100 text-purple-700 hover:bg-purple-200 border-purple-200';
      case 'Rejected': return 'bg-red-100 text-red-700 hover:bg-red-200 border-red-200';
      case 'Selected': return 'bg-green-100 text-green-700 hover:bg-green-200 border-green-200';
