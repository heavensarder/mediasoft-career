...
export default function InterviewPanelClient({
  ...
  const [selectedAppInterviewerId, setSelectedAppInterviewerId] = useState<
    number | null
  >(null);
  ...
  const handleEndInterview = async () => {
    const jobId =
      selectedPosition === "all" ? null : parseInt(selectedPosition);
  ...
  const renderApplicantRow = (app: Application) => (
    <TableRow key={app.id} className="hover:bg-white/30 transition-colors">
      ...
      <TableCell
        className={`text-center ${app.interviewMarking?.excludeWritten ? "opacity-50" : getWrittenExamColor(app.interviewMarking?.writtenExam ?? null)}`}
      >
        {app.interviewMarking?.excludeWritten ? (
          <span
            className="line-through text-slate-400"
            title="Excluded from total"
          >
            N/A
          </span>
        ) : (
      ...
      <TableCell
        className={`text-center ${app.interviewMarking?.excludeTechnical ? "opacity-50" : getTechnicalVivaColor(app.interviewMarking?.technicalViva ?? null)}`}
      >
        {app.interviewMarking?.excludeTechnical ? (
          ...
          (app.interviewMarking?.technicalViva ?? (
            <span className="text-muted-foreground">—</span>
          ))
        )}
      </TableCell>
      ...
      <TableCell>
        {isAdmin ? (
          <Select
            value={app.status}
            onValueChange={(value) =>
              handleStatusChange(app.id, value, app.fullName)
            }
          >
            <SelectTrigger
              className={`w-[120px] h-8 text-xs font-medium border ${getStatusColor(app.status)}`}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="flex justify-end gap-2">
          {isAdmin && (
            <Button
              variant="outline"
              size="sm"
              className={`text-xs border-0 text-white hover:text-white shadow-sm ${
                app.assignedInterviewerIds?.length > 0
              ...
              onClick={() => {
                setAssigningApp(app);
                setSelectedInterviewers(
                  app.assignedInterviewerIds?.map((id) => id.toString()) || [],
                );
            ...
            >
              ...
              {app.assignedInterviewerIds?.length > 0
                ? `Assigned (${app.assignedInterviewerIds.length})`
                : "Assign"}
            </Button>
          )}
          {isAdmin && (
            <Link
              href={`/admin/dashboard/job-recruitment/applications/${app.id}`}
            >
              <Button
                variant="outline"
                size="sm"
  ...
  const renderApplicantCard = (app: Application) => (
    <div
      key={app.id}
      className="premium-glass-card p-4 hover:shadow-xl transition-all"
    >
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <Badge
            className={`${getStatusColor(app.status)} mt-2 border text-xs`}
          >
            {app.status}
          </Badge>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center mb-4 p-3 bg-white/50 rounded-lg">
        <div
          className={app.interviewMarking?.excludeWritten ? "opacity-50" : ""}
        >
          <p className="text-xs text-slate-500">Written</p>
          {app.interviewMarking?.excludeWritten ? (
            <p
              className="font-bold line-through text-slate-400"
              title="Excluded"
            >
              N/A
            </p>
            ...
            <p
              className={`font-bold ${getWrittenExamColor(app.interviewMarking?.writtenExam ?? null)}`}
            >
              {app.interviewMarking?.writtenExam ?? "—"}
            </p>
          )}
        ...
        <div
          className={app.interviewMarking?.excludeTechnical ? "opacity-50" : ""}
        >
          ...
          {app.interviewMarking?.excludeTechnical ? (
            ...
            <p
              className={`font-bold ${getTechnicalVivaColor(app.interviewMarking?.technicalViva ?? null)}`}
            >
              {app.interviewMarking?.technicalViva ?? "—"}
            </p>
          )}
        ...
        <div
          className={app.interviewMarking?.excludeProject ? "opacity-50" : ""}
        >
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {isAdmin && (
          ...
          >
            <Button
              variant="outline"
              size="sm"
              className="w-full text-xs bg-gradient-to-r from-slate-500 to-slate-600 border-0 text-white hover:text-white hover:from-slate-600 hover:to-slate-700 shadow-sm"
            >
              View Details
            </Button>
          </Link>
        ...
        {isMarkingEnabled(app.interviewDate) ? (
          <Link
            href={`/admin/dashboard/interview-panel/marking/${app.id}`}
            className="flex-1"
          >
            <Button
              variant="outline"
              size="sm"
              className="w-full text-xs bg-gradient-to-r from-blue-500 to-indigo-600 border-0 text-white hover:text-white hover:from-blue-600 hover:to-indigo-700 shadow-sm"
            >
              Set Mark
            </Button>
          </Link>
        ...
        <div className="space-y-6">
          {jobPositions.map((position) => (
            <Collapsible
              key={position}
              open={
                expandedSections.has(position) || expandedSections.size === 0
              }
                  ...
                  <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-white/30 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant="secondary">
                        {groupedByPosition[position].length} applicants
                      </Badge>
                    </div>
                    {isAdmin && (
                      <div
                      ...
                      >
                        <Button
                          ...
                          onClick={() => {
                            const jobId = applications.find(
                              (a) => a.job.title === position,
                            )?.job.id;
                            if (jobId) {
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{pendingReject?.name}</strong>? This will update their
              status to Rejected.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setPendingReject(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog open={endInterviewOpen} onOpenChange={setEndInterviewOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove applicants from the Interview Panel (change
              status from "Interview"). Select which position to end interviews
              for.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
          ...
          <div className="bg-gradient-to-r from-violet-500 to-purple-600 px-6 py-5 text-white">
            ...
            <AlertDialogDescription className="text-violet-100 mt-2 text-sm">
              ...
              <span className="font-semibold text-white">
                {assigningApp?.fullName}
              </span>{" "}
              to one or more interviewers. Selected interviewers will be able to
              see and mark this applicant.
            </AlertDialogDescription>
          </div>
              ...
              {interviewers.map((i, index) => {
                ...
                return (
                  ...
                  >
                    {/* Custom Checkbox */}
                    <div
                      className={`relative flex-shrink-0 w-6 h-6 rounded-lg border-2 transition-all duration-200 flex items-center justify-center ${
                        isSelected
