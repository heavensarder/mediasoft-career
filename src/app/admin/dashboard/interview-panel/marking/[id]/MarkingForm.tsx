...
import { Loader2, Save, FileText, Code, FolderKanban, ShieldAlert } from 'lucide-react';
...
export default function MarkingForm({
    ...
    // Calculate total only for visible sections
    const totalScore = (hasWrittenAccess ? writtenExam : 0) +
        (hasTechnicalAccess ? technicalViva : 0) +
        (hasProjectAccess ? (projectRating * 2) : 0);
    const maxScore = (hasWrittenAccess ? 30 : 0) +
        (hasTechnicalAccess ? 10 : 0) +
        (hasProjectAccess ? 10 : 0);
                ...
                <SectionCard title="Written Exam" icon={FileText}>
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className={`text-2xl font-bold ${writtenExam >= 15 ? 'text-green-600' : 'text-red-600'}`}>{writtenExam}</span>
                        </div>
                        <Slider
                            value={[writtenExam]}
                ...
                <SectionCard title="Technical Viva" icon={Code}>
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className={`text-2xl font-bold ${technicalViva >= 6 ? 'text-green-600' : 'text-red-600'}`}>{technicalViva}</span>
                        </div>
                        <Slider
                            value={[technicalViva]}
